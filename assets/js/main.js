   ...
   if ($('.accordion-box').length) {
    ...
    ================================ */
       ...
       if($('.banner-active').length > 0) {
        ...
        //>> Floating Leaves Animation Disabled <<//
        /*
        if ($(".has-floating-leaves").length > 0) {
            $(".has-floating-leaves").each(function() {
                var $container = $('<div class="floating-leaves-container"></div>');
                $(this).append($container);
                for (var i = 0; i < 15; i++) {
                    var $leaf = $('<div class="floating-leaf"></div>');
                    var leftPos = Math.random() * 100;
                    var delay = Math.random() * 10;
                    var duration = 8 + Math.random() * 10;
                    var scale = 0.5 + Math.random() * 0.8;
                    var animClass = 'leaf-animate-' + (Math.floor(Math.random() * 4) + 1);
